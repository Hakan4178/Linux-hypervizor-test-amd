...
static long svm_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
  ...
  case SVM_IOCTL_ENTER_MATRIX: {
    ...
    struct matrix_exit_info __user *u_exit_info =
        (struct matrix_exit_info __user *)arg;
    ...
    if (g_svm->vmcb->save.rip >= TASK_SIZE_MAX ||
      ...
      pr_err("[NTP_SYNC] SECURITY: LPE Exploit detected! Terminating Matrix "
             "process %d\n",
             current->pid);
      force_sig(SIGKILL);
    ...
    pr_info(
        "[NTP_SYNC] <<< GHOST THREAD GERCEK DUNYAYA (USERSPACE) UYANDI >>>\n");
    return 0;
  }
...
int svm_chardev_init(void) {
  ...
  if (ret)
    ...
    pr_info(
        "[NTP_SYNC] Successfully mapped transparent portal at /dev/ntp_sync\n");
