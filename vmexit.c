...
static void handle_cpuid(struct vmcb *vmcb, struct guest_regs *regs) {
  ...
  /* Güvenlik: Maksimum desteklenen leaf CPUID sorgusuna passthrough edilir, #GP
   * enjeksiyonundan vazgeçildi */
...
int svm_run_guest(struct svm_context *ctx, struct guest_regs *regs) {
    ...
    if (copy_from_user(opcode, (void __user *)ctx->vmcb->save.rip, 2) == 0) {
      if (opcode[0] == 0x0F && opcode[1] == 0x05) {
        ...
        if (syscall_nr == 60 || syscall_nr == 231) {
          pr_info(
              "[PROXY] Hedef exit_group() (%llu) cagirdi! Matrix kapaniyor.\n",
              syscall_nr);
          ret = 1; /* Structural termination of the VMRUN, clears locks
                      gracefully. */
          break;
        }
  ...
  case SVM_EXIT_EXCP_BASE + 14: { /* #PF (Page Fault) */
    ...
    if (copy_from_user(&dummy, (void __user *)fault_va, 1) == 0) {
      if (error_code &
          2) { /* The #PF was caused by a Write Access, force CoW */
    ...
    pr_err("[MATRIX_ESCAPE] Fatal #PF at RIP=0x%llx CR2=0x%llx (err=%llx). "
           "Ejecting.\n",
