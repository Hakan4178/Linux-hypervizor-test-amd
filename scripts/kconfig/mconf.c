...
static const char
    mconf_readme[] =
        ...
        "Search\n"
        "-------\n"
        "Pressing the forward-slash (/) anywhere brings up a search dialog "
        "box.\n"
        "\n"
...
static int show_textbox_ext(const char *title, const char *text, int r, int c,
                            ...
                            void *data) {
  dialog_clear();
  return dialog_textbox(title, text, r, c, vscroll, hscroll, extra_key_cb,
                        data);
}
...
again:
  ...
  do {
    ...
    set_subtitle();
    dres = show_textbox_ext("Search Results", str_get(&res), 0, 0, &vscroll,
                            &hscroll, handle_search_keys, &data);
    again = false;
    if (dres >= '1' && dres <= '9') {
...
static void conf_choice(struct menu *menu) {
  ...
  while (1) {
    ...
    res = dialog_checklist(prompt ? prompt : "Main Menu",
                           radiolist_instructions, MENUBOX_HEIGHT_MIN,
