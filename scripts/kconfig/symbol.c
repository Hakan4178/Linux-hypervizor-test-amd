...
static void sym_set_all_changed(void) {
  ...
  for_all_symbols(sym) sym_set_changed(sym);
}
...
struct symbol *sym_choice_default(struct menu *choice) {
  ...
  menu_for_each_sub_entry(
      menu, choice) if (menu->sym && menu->sym->visible != no) return menu->sym;
...
struct symbol *sym_calc_choice(struct menu *choice) {
   ...
   */
  if (!res) {
    list_for_each_entry_reverse(sym, &choice->choice_members, choice_link) {
      if (sym->visible == no)
        continue;
  ...
  if (name) {
    ...
    hash_for_each_possible(sym_hashtable, symbol, node, hash) {
      if (symbol->name && !strcmp(symbol->name, name) &&
          (flags ? symbol->flags & flags : !(symbol->flags & SYMBOL_CONST)))
        return symbol;
    }
  ...
  for (; stack; stack = stack->next) {
    ...
    if (sym_is_choice(next_sym)) {
      ...
      fprintf(stderr, "\tsymbol %s is part of choice block at %s:%d\n",
              sym->name ? sym->name : "<choice>", choice->filename,
              choice->lineno);
    } else if (stack->expr == &sym->dir_dep.expr) {
      fprintf(stderr, "\tsymbol %s depends on %s\n",
              sym->name ? sym->name : "<choice>", next_sym->name);
      ...
      fprintf(stderr, "\tsymbol %s %s value contains %s\n",
              sym->name ? sym->name : "<choice>", prop_get_type_name(type),
              next_sym->name);
    } else {
...
out:
  menu_for_each_sub_entry(menu, choice_menu) if (menu->sym) menu->sym->flags &=
      ~SYMBOL_CHECK;
