...
enum view_mode { SINGLE_VIEW, SPLIT_VIEW, FULL_VIEW };
...
static void set_node(GtkTreeStore *tree, GtkTreeIter *node, struct menu *menu) {
  ...
  option = g_strdup_printf("%s %s %s %s", menu->type == M_COMMENT ? "***" : "",
...
set:
  gtk_tree_store_set(
      tree, node, COL_OPTION, option, COL_NAME, sym ? sym->name : "", COL_NO,
      _no, COL_MOD, _mod, COL_YES, _yes, COL_VALUE, value, COL_MENU,
      (gpointer)menu, COL_COLOR, &color, COL_EDIT, editable, COL_PIXBUF,
      pix_menu, COL_PIXVIS, view_mode == SINGLE_VIEW && menu->type == M_MENU,
      COL_BTNVIS, btnvis, COL_BTNACT, _yes[0] == 'Y', COL_BTNINC,
      _mod[0] == 'M', COL_BTNRAD, sym && sym_is_choice_value(sym), -1);
...
static void on_save_as1_activate(GtkMenuItem *menuitem, gpointer user_data) {
  ...
  dialog = gtk_file_chooser_dialog_new(
      "Save file as...", GTK_WINDOW(user_data), GTK_FILE_CHOOSER_ACTION_SAVE,
...
static void on_introduction1_activate(GtkMenuItem *menuitem,
                                      gpointer user_data) {
  GtkWidget *dialog;
...
static gboolean on_window1_configure(GtkWidget *self, GdkEventConfigure *event,
                                     gpointer user_data) {
...
static gboolean on_window1_delete_event(GtkWidget *widget, GdkEvent *event,
  ...
  dialog = gtk_dialog_new_with_buttons(
      ...
      "_OK", GTK_RESPONSE_YES, "_No", GTK_RESPONSE_NO, "_Cancel",
...
static void init_main_window(const gchar *glade_file) {
  ...
  g_signal_connect(main_wnd, "destroy", G_CALLBACK(on_window1_destroy), NULL);
  ...
  g_signal_connect(save_menu_item, "activate", G_CALLBACK(on_save_activate),
                   NULL);
  ...
  g_signal_connect(widget, "activate", G_CALLBACK(on_show_name1_activate),
                   NULL);
  ...
  g_signal_connect(widget, "activate", G_CALLBACK(on_show_range1_activate),
                   NULL);
  ...
  g_signal_connect(widget, "activate", G_CALLBACK(on_show_data1_activate),
                   NULL);
  ...
  g_signal_connect(widget, "activate", G_CALLBACK(on_set_option_mode1_activate),
                   NULL);

  widget = GTK_WIDGET(gtk_builder_get_object(builder, "set_option_mode2"));
  g_signal_connect(widget, "activate", G_CALLBACK(on_set_option_mode2_activate),
                   NULL);

  widget = GTK_WIDGET(gtk_builder_get_object(builder, "set_option_mode3"));
  g_signal_connect(widget, "activate", G_CALLBACK(on_set_option_mode3_activate),
                   NULL);

  widget = GTK_WIDGET(gtk_builder_get_object(builder, "introduction1"));
  g_signal_connect(widget, "activate", G_CALLBACK(on_introduction1_activate),
                   NULL);
...
static void init_left_tree(void) {
  ...
  tree1 = gtk_tree_store_new(
      ...
      G_TYPE_STRING, G_TYPE_STRING, G_TYPE_POINTER, GDK_TYPE_RGBA,
  ...
  gtk_tree_view_column_pack_start(GTK_TREE_VIEW_COLUMN(column), renderer,
                                  FALSE);
  gtk_tree_view_column_set_attributes(GTK_TREE_VIEW_COLUMN(column), renderer,
                                      "text", COL_OPTION, "foreground-rgba",
                                      COL_COLOR, NULL);
