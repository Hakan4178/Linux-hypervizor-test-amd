...
static struct expr *rewrite_m(struct expr *e) {
  ...
  case E_OR:
  case E_AND:
    e = expr_alloc_two(e->type, rewrite_m(e->left.expr),
...
void menu_add_dep(struct expr *dep, struct expr *cond) {
  if (cond) {
     ...
     */
    dep = expr_alloc_or(dep, expr_trans_compare(cond, E_EQUAL, &symbol_no));
  }
...
void menu_finalize(void) { _menu_finalize(&rootmenu, false); }
...
int __attribute__((weak)) get_jump_key_char(void) { return -1; }
