...
static int fn_is_fake_mcount(Elf_Rel const *rp) { return 0; }
...
static void (*Elf_r_info)(Elf_Rel *const rp, unsigned sym,
                          unsigned type) = fn_ELF_R_INFO;
...
static int append_func(Elf_Ehdr *const ehdr, Elf_Shdr *const shstr,
                       uint_t const *const mloc0, uint_t const *const mlocp,
                       Elf_Rel const *const mrel0, Elf_Rel const *const mrelp,
                       unsigned int const rel_entsize,
                       unsigned int const symsec_sh_link) {
  /* Begin constructing output file */
  Elf_Shdr mcsec;
  char const *mc_name = (sizeof(Elf_Rela) == rel_entsize) ? ".rela__mcount_loc"
                                                          : ".rel__mcount_loc";
  ...
  if (uwrite(old_shoff + (void *)ehdr, sizeof(Elf_Shdr) * old_shnum) < 0)
    return -1;

  /* new sections __mcount_loc and .rel__mcount_loc */
  t += 2 * sizeof(mcsec);
  mcsec.sh_name =
      w((sizeof(Elf_Rela) == rel_entsize) + strlen(".rel") + old_shstr_sh_size);
  ...
  mcsec.sh_type = (sizeof(Elf_Rela) == rel_entsize) ? w(SHT_RELA) : w(SHT_REL);
  mcsec.sh_flags = 0;
  mcsec.sh_addr = 0;
...
static unsigned get_mcountsym(Elf_Sym const *const sym0, Elf_Rel const *relp,
...
static void get_sym_str_and_relp(Elf_Shdr const *const relhdr,
                                 ...
                                 Elf_Sym const **sym0, char const **str0,
                                 Elf_Rel const **relp) {
  ...
  Elf_Rel const *const rel0 =
      (Elf_Rel const *)(_w(relhdr->sh_offset) + (void *)ehdr);

  *sym0 = (Elf_Sym const *)(_w(symsec->sh_offset) + (void *)ehdr);

  *str0 = (char const *)(_w(strsec->sh_offset) + (void *)ehdr);
 ...
 */
static uint_t *sift_rel_mcount(uint_t *mlocp, unsigned const offbase,
                               Elf_Rel **const mrelpp,
                               Elf_Shdr const *const relhdr,
                               Elf_Ehdr const *const ehdr,
                               unsigned const recsym, uint_t const recval,
 ...
 * into nops.
 */
static int nop_mcount(Elf_Shdr const *const relhdr, Elf_Ehdr const *const ehdr,
 ...
 */
static int find_secsym_ndx(unsigned const txtndx, char const *const txtname,
                           uint_t *const recvalp, unsigned int *sym_index,
                           Elf_Shdr const *const symhdr,
                           Elf32_Word const *symtab,
                           Elf32_Word const *symtab_shndx,
                           Elf_Ehdr const *const ehdr) {
  Elf_Sym const *const sym0 =
      (Elf_Sym const *)(_w(symhdr->sh_offset) + (void *)ehdr);
...
static unsigned tot_relsize(Elf_Shdr const *const shdr0, unsigned nhdr,
...
static int do_func(Elf_Ehdr *const ehdr, char const *const fname,
  ...
  char const *const shstrtab =
      (char const *)(_w(shstr->sh_offset) + (void *)ehdr);
  ...
  for (relhdr = shdr0, k = nhdr; k; --k, ++relhdr) {
    char const *const txtname = has_rel_mcount(relhdr, shdr0, shstrtab, fname);
    if (txtname == already_has_rel_mcount) {
      result = 0;
      file_updated = 0;
    ...
    if (txtname && is_mcounted_section_name(txtname)) {
      ...
      result =
          find_secsym_ndx(w(relhdr->sh_info), txtname, &recval, &recsym,
                          &shdr0[symsec_sh_link], symtab, symtab_shndx, ehdr);
      if (result)
        goto out;

      rel_entsize = _w(relhdr->sh_entsize);
      mlocp = sift_rel_mcount(mlocp, (void *)mlocp - (void *)mloc0, &mrelp,
  ...
  if (!result && mloc0 != mlocp)
    result = append_func(ehdr, shstr, mloc0, mlocp, mrel0, mrelp, rel_entsize,
                         symsec_sh_link);
out:
  free(mrel0);
  free(mloc0);
