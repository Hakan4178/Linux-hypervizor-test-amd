...
static void release_file(void *file, size_t size) { munmap(file, size); }
...
static void default_mismatch_handler(const char *modname, struct elf_info *elf,
   ...
   * <address>
   */
  warn("%s: section mismatch in reference: %s%s0x%x (section: %s) -> %s "
       "(section: %s)\n",
...
static void check_export_symbol(struct module *mod, struct elf_info *elf,
  ...
  if (strcmp(label_name + strlen(prefix), name)) {
    error("%s: .export_symbol section references '%s', but it does not seem to "
          "be an export symbol\n",
          mod->name, name);
    return;
  }
  ...
  if (match(secname, PATTERNS(ALL_INIT_SECTIONS)))
    warn("%s: %s: EXPORT_SYMBOL used for init symbol. Remove __init or "
         "EXPORT_SYMBOL.\n",
         mod->name, name);
  else if (match(secname, PATTERNS(ALL_EXIT_SECTIONS)))
    warn("%s: %s: EXPORT_SYMBOL used for exit symbol. Remove __exit or "
         "EXPORT_SYMBOL.\n",
         mod->name, name);
}
...
static Elf_Addr addend_arm_rel(void *loc, Elf_Sym *sym, unsigned int r_type) {
  ...
  case R_ARM_THM_MOVT_ABS:
    ...
    offset = sign_extend32(((upper & 0x000f) << 12) | ((upper & 0x0400) << 1) |
  ...
  case R_ARM_THM_JUMP24:
    ...
    offset = sign_extend32(
        (sign << 24) | ((~(j1 ^ sign) & 1) << 23) | ((~(j2 ^ sign) & 1) << 22) |
...
int main(int argc, char **argv) {
  ...
  if (nr_unresolved > MAX_UNRESOLVED_REPORTS)
    warn("suppressed %u unresolved symbol warnings because there were too "
         "many)\n",
