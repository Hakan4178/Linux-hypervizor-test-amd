...
#define bswap(x)                                                               \
  ({                                                                           \
    _Static_assert(sizeof(x) == 1 || sizeof(x) == 2 || sizeof(x) == 4 ||       \
                       sizeof(x) == 8,                                         \
                   "bug");                                                     \
...
#define TO_NATIVE(x) (target_is_big_endian == host_is_big_endian ? x : bswap(x))
...
void __attribute__((format(printf, 2, 3))) modpost_log(bool is_error,
                                                       const char *fmt, ...);
