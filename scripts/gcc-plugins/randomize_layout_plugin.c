 ...
 * <spender@grsecurity.net> and PaX Team <pageexec@freemail.hu> Licensed under
 * the GPL v2
 *
 ...
 * randomize_layout_plugin.so randomize_layout_plugin.c $ # for C++
 * based 4.7/4.8+ $ g++ -I`g++ -print-file-name=plugin`/include -I`g++
 * -print-file-name=plugin`/include/c-family -fPIC -shared -O2 -o
 * randomize_layout_plugin.so randomize_layout_plugin.c $ gcc
 * -fplugin=./randomize_layout_plugin.so test.c -O2
 */
...
static void check_bad_casts_in_constructor(tree var, tree init) {
  ...
  FOR_EACH_CONSTRUCTOR_ELT(CONSTRUCTOR_ELTS(init), idx, field, val) {
    ...
    field_type = TYPE_MAIN_VARIANT(
        strip_array_types(TYPE_MAIN_VARIANT(TREE_TYPE(field_type))));
    val_type = TYPE_MAIN_VARIANT(
        strip_array_types(TYPE_MAIN_VARIANT(TREE_TYPE(val_type))));
...
static unsigned int find_bad_casts_execute(void) {
  ...
  FOR_EACH_BB_FN(bb, cfun) {
    ...
    for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
      ...
      ptr_lhs_type = TYPE_MAIN_VARIANT(
          strip_array_types(TYPE_MAIN_VARIANT(TREE_TYPE(lhs_type))));
      ptr_rhs_type = TYPE_MAIN_VARIANT(
          strip_array_types(TYPE_MAIN_VARIANT(TREE_TYPE(rhs_type))));
      ...
      if (rhs_code == ADDR_EXPR) {
        ...
        op0_type = TYPE_MAIN_VARIANT(
            strip_array_types(TYPE_MAIN_VARIANT(TREE_TYPE(op0))));
...
__visible int plugin_init(struct plugin_name_args *plugin_info,
  ...
  if (strncmp(lang_hooks.name, "GNU C", 5) &&
      !strncmp(lang_hooks.name, "GNU C+", 6)) {
  ...
  obtained_seed =
      sscanf(randstruct_seed, "%016llx%016llx%016llx%016llx", &shuffle_seed[0],
             &shuffle_seed[1], &shuffle_seed[2], &shuffle_seed[3]);
  if (obtained_seed != 4) {
