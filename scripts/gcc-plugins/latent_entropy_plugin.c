...
static tree handle_latent_entropy_attribute(tree *node, tree name,
  ...
  case VAR_DECL:
    if (DECL_INITIAL(*node)) {
      *no_add_attrs = true;
      error("variable %qD with %qE attribute must not be initialized", *node,
            name);
      break;
    }
    ...
    default:
      *no_add_attrs = true;
      error("variable %qD with %qE attribute must be an integer or a fixed "
            "length integer array type or a fixed sized structure with integer "
            "fields",
            *node, name);
      break;
    ...
    case RECORD_TYPE: {
      ...
      for (fld = lst; fld; nelt++, fld = TREE_CHAIN(fld)) {
        ...
        *no_add_attrs = true;
        error("structure variable %qD with %qE attribute has a non-integer "
              "field %qE",
              *node, name, fld);
        break;
      }
    ...
    case ARRAY_TYPE: {
      ...
      if (TREE_CODE(elt_type) != INTEGER_TYPE || !array_size ||
        ...
        *no_add_attrs = true;
        error("array variable %qD with %qE attribute must be a fixed length "
              "integer array type",
              *node, name);
        break;
      }
...
static gimple create_assign(enum tree_code code, tree lhs, tree op1, tree op2) {
...
__visible int plugin_init(struct plugin_name_args *plugin_info,
  ...
  if (enabled) {
    ...
    register_callback(plugin_name, PLUGIN_REGISTER_GGC_ROOTS, NULL,
                      (void *)&gt_ggc_r_gt_latent_entropy);
