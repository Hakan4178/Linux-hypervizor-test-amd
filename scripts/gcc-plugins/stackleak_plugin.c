...
static struct plugin_info stackleak_plugin_info = {
    ...
    .help = "track-min-size=nn\ttrack stack for functions with a stack frame "
            "size >= nn bytes\n"
...
static void add_stack_tracking_gasm(gimple_stmt_iterator *gsi, bool after) {
  ...
  asm_call = gimple_build_asm_vec("call __sanitizer_cov_stack_depth", inputs,
                                  NULL, NULL, NULL);
...
static unsigned int stackleak_instrument_execute(void) {
   ...
   */
  if (is_leaf && !TREE_PUBLIC(current_function_decl) &&
      DECL_DECLARED_INLINE_P(current_function_decl)) {
    return 0;
  }

  if (is_leaf && !strncmp(IDENTIFIER_POINTER(DECL_NAME(current_function_decl)),
                          "_paravirt_", 10)) {
    return 0;
  }
 ...
 */
static bool stackleak_instrument_gate(void) { return stackleak_gate(); }
...
static bool stackleak_cleanup_gate(void) { return stackleak_gate(); }
...
__visible int plugin_init(struct plugin_name_args *plugin_info,
   ...
   */
  PASS_INFO(stackleak_instrument, "optimized", 1, PASS_POS_INSERT_BEFORE);
  ...
  register_callback(plugin_name, PLUGIN_START_UNIT, &stackleak_start_unit,
                    NULL);
