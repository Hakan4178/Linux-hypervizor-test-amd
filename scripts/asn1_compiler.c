    ...
    while (p < nl) {
      ...
      if (isalpha(*p)) {
        ...
        dir = bsearch(&tokens[tix], directives,
                      ...
                      sizeof(directives[1]), directive_compare);
        if (dir) {
...
static struct element *parse_type(struct token **_cursor, struct token *end,
  ...
  if (cursor < end && cursor->token_type == TOKEN_OPEN_ACTION) {
    ...
    for (ppaction = &action_list; *ppaction; ppaction = &(*ppaction)->next) {
...
static void dump_element(const struct element *e, int level) {
  ...
  if (e->class == 0 && e->method == 0 && e->tag == 0)
    ...
    sprintf(tag, "%s %s %s", asn1_classes[e->class], asn1_methods[e->method],
            asn1_universal_tags[e->tag]);
  else
    sprintf(tag, "%s %s %u", asn1_classes[e->class], asn1_methods[e->method],
            e->tag);
  ...
  printf("%c%c%c%c%c %c %*s[*] \e[33m%s\e[m %s %s \e[35m%s\e[m\n",
         ...
         e->flags & ELEMENT_CONDITIONAL ? 'C' : '-', "-tTqQcaro"[e->compound],
         level, "", tag, tname, name, e->action ? e->action->name : "");
...
static void render_element(FILE *out, struct element *e, struct element *tag) {
  ...
  case SET:
  case SET_OF:
    render_opcode(out, "ASN1_OP_%sMATCH%s%s,", cond, outofline ? "_JUMP" : "",
                  skippable ? "_OR_SKIP" : "");
    break;
  ...
  case SEQUENCE:
    if (outofline) {
      ...
      if (e->type_def && e->type_def->name)
        render_more(out, "\t\t// --> %s", e->type_def->name->content);
      render_more(out, "\n");
