...
#define DEFINE_GET_ATTR(attr, type)                                            \
  static bool get_##attr##_attr(Dwarf_Die *die, unsigned int id,               \
                                type *value) {                                 \
    Dwarf_Attribute da;                                                        \
    return dwarf_attr(die, id, &da) && !dwarf_form##attr(&da, value);          \
  }

DEFINE_GET_ATTR(flag, bool)
DEFINE_GET_ATTR(udata, Dwarf_Word)

static bool get_ref_die_attr(Dwarf_Die *die, unsigned int id,
                             Dwarf_Die *value) {
  Dwarf_Attribute da;
...
static void update_fqn(struct die *cache, Dwarf_Die *die) {
  ...
  if (!cache->fqn) {
    if (!__die_map_get((uintptr_t)die->addr, DIE_FQN, &fqn) && *fqn->fqn)
...
#define DEFINE_PROCESS_UDATA_ATTRIBUTE(attribute)                              \
  static void process_##attribute##_attr(struct die *cache, Dwarf_Die *die) {  \
    Dwarf_Word value;                                                          \
    if (get_udata_attr(die, DW_AT_##attribute, &value))                        \
      process_fmt(cache, " " #attribute "(%" PRIu64 ")", value);               \
  }
...
bool match_all(Dwarf_Die *die) { return true; }
...
#define DEFINE_PROCESS_LIST_TYPE(type)                                         \
  static void process_##type##_type(struct state *state, struct die *cache,    \
                                    Dwarf_Die *die) {                          \
...
static void process_variant_part_type(struct state *state, struct die *cache,
  ...
  process_linebreak(cache, 1);
  check(process_die_container(state, cache, die, process_type, match_all));
  process_linebreak(cache, -1);
  process(cache, "}");
}
...
static int get_kabi_status(Dwarf_Die *die, const char **suffix) {
  ...
  if (is_kabi_prefix(name)) {
    ...
    if (!strncmp(name, KABI_RESERVED_PREFIX, KABI_RESERVED_PREFIX_LEN))
      return KABI_RESERVED;
    if (!strncmp(name, KABI_IGNORED_PREFIX, KABI_IGNORED_PREFIX_LEN))
      return KABI_IGNORED;

    if (!strncmp(name, KABI_RENAMED_PREFIX, KABI_RENAMED_PREFIX_LEN)) {
      if (suffix) {
...
static int check_union_member_kabi_status(struct state *state,
  ...
  if (dwarf_tag(&type) == DW_TAG_structure_type)
    res = checkp(process_die_container(state, NULL, &type,
...
static int get_union_kabi_status(Dwarf_Die *die, Dwarf_Die *placeholder,
  ...
  res = checkp(process_die_container(
      &state, NULL, die, check_union_member_kabi_status, match_member_type));
...
static void __process_structure_type(struct state *state, struct die *cache,
  ...
  if (expand) {
    ...
    check(process_die_container(state, cache, die, process_func, match_func));
  }
...
#define DEFINE_PROCESS_STRUCTURE_TYPE(structure)                               \
  static void process_##structure##_type(struct state *state,                  \
                                         struct die *cache, Dwarf_Die *die) {  \
    __process_structure_type(state, cache, die, #structure "_type",            \
...
static void process_union_type(struct state *state, struct die *cache,
  ...
  int res =
      checkp(get_union_kabi_status(die, &placeholder, &state->kabi.orig_name));
...
static void process_enumerator_type(struct state *state, struct die *cache,
  ...
  if (stable) {
    ...
    if (kabi_is_enumerator_ignored(state->expand.current_fqn, cache->fqn))
      return;

    overridden = kabi_get_enumerator_value(state->expand.current_fqn,
                                           cache->fqn, &override);
    value = override;
  }
...
static void process_enumeration_type(struct state *state, struct die *cache,
                                     Dwarf_Die *die) {
  __process_structure_type(state, cache, die, "enumeration_type", process_type,
...
static void process_cached(struct state *state, struct die *cache,
  ...
  list_for_each_entry(df, &cache->fragments, list) {
    ...
    case FRAGMENT_STRING:
      die_debug_b("cache %p STRING '%s'", cache, df->data.str);
    ...
    case FRAGMENT_LINEBREAK:
      ...
      break;
    case FRAGMENT_DIE:
      if (!dwarf_die_addr_die(dwarf_cu_getdwarf(die->cu), (void *)df->data.addr,
                              &child))
        error("dwarf_die_addr_die failed");
      die_debug_b("cache %p DIE addr %" PRIxPTR " tag %x", cache, df->data.addr,
                  dwarf_tag(&child));
...
static int process_type(struct state *state, struct die *parent,
  ...
  die_debug_r("parent %p cache %p die addr %p tag %x", parent, cache, die->addr,
              tag);
...
static void save_symbol_ptr(struct state *state) {
  ...
  if (!get_ref_die_attr(&state->die, DW_AT_type, &ptr_type) ||
    ...
    error("%s must be a pointer type!", get_symbol_name(&state->die));

  if (!get_ref_die_attr(&ptr_type, DW_AT_type, &type))
    error("%s pointer missing a type attribute?", get_symbol_name(&state->die));
...
static int process_exported_symbols(struct state *unused, struct die *cache,
  ...
  case DW_TAG_class_type:
  case DW_TAG_structure_type:
    return check(process_die_container(NULL, cache, die,
...
static void process_symbol_ptr(struct symbol *sym, void *arg) {
  ...
  if (!dwarf_die_addr_die(dwarf, (void *)sym->ptr_die_addr, &state.die))
    error("dwarf_die_addr_die failed for symbol ptr: '%s'", sym->name);
...
static int resolve_fqns(struct state *parent, struct die *unused,
  ...
  check(process_die_container(&state, NULL, die, resolve_fqns, match_all));
...
void process_cu(Dwarf_Die *cudie) {
  check(process_die_container(NULL, NULL, cudie, resolve_fqns, match_all));
