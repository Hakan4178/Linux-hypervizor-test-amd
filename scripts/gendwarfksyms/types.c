...
static int type_list_append(struct list_head *list, const char *s,
                            void *owned) {
  struct type_list_entry *entry;
...
static void type_map_write(FILE *file) {
  ...
  hash_for_each_safe(type_map, e, tmp, hash)++ count;
  es = xmalloc(count * sizeof(*es));
  hash_for_each_safe(type_map, e, tmp, hash) es[i++] = e;
  ...
  list_for_each_entry(entry, &type->expanded, list) {
    if (is_type_prefix(entry->str)) {
      if (type_map_get(entry->str, &e))
        error("unknown type reference to '%s' when expanding '%s'", entry->str,
              type->name);
...
static void __type_expand(struct die *cache, struct type_expansion *type) {
  ...
  list_for_each_entry(df, &cache->fragments, list) {
    ...
    case FRAGMENT_DIE:
      ...
      if (__die_map_get(df->data.addr, DIE_COMPLETE, &child) &&
          __die_map_get(df->data.addr, DIE_UNEXPANDED, &child))
        error("unknown child: %" PRIxPTR, df->data.addr);
    ...
    case FRAGMENT_LINEBREAK:
      ...
      if (list_is_last(&df->list, &cache->fragments) ||
          list_next_entry(df, list)->type != FRAGMENT_LINEBREAK)
...
static void type_parse(const char *name, const char *str,
  ...
  for (pos = 0; str[pos]; ++pos) {
    ...
    if (empty)
      error("empty %c# type name for '%s' (string: '%s')", str[pos], name, str);
