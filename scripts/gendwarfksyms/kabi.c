...
#define KABI_RULE_MIN_ENTRY_SIZE                                               \
  (/* version\0 */ 2 + /* type\0 */ 2 + /* target\0" */ 1 + /* value\0 */ 1)
...
static struct rule *find_rule(enum kabi_rule_type type, const char *target) {
  ...
  hash_for_each_possible(rules, rule, hash, rule_values_hash(type, target)) {
...
bool kabi_is_enumerator_ignored(const char *fqn, const char *field) {
  return !!find_enumerator_rule(KABI_RULE_TYPE_ENUMERATOR_IGNORE, fqn, field);
}
...
bool kabi_get_enumerator_value(const char *fqn, const char *field,
  ...
  rule = find_enumerator_rule(KABI_RULE_TYPE_ENUMERATOR_VALUE, fqn, field);
  if (rule) {
