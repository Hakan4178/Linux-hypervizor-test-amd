...
static void set_crc(struct symbol *sym, void *data) {
  ...
  if (sym->state == SYMBOL_PROCESSED && sym->crc != *crc)
    warn("overriding version for symbol %s (crc %lx vs. %lx)", sym->name,
         sym->crc, *crc);
...
void symbol_for_each(symbol_callback_t func, void *arg) {
  ...
  hash_for_each_safe(symbol_names, sym, tmp, name_hash) { func(sym, arg); }
}
  ...
  while (scn) {
    ...
    if (shdr->sh_type == SHT_SYMTAB) {
      ...
      if (shdr->sh_entsize != sym_size)
        error("expected sh_entsize (%" PRIu64 ") to be %zu", shdr->sh_entsize,
              sym_size);
      ...
      for (n = 1; n < nsyms; ++n) {
        ...
        sym = gelf_getsymshndx(data, xndx_data, n, &sym_mem, &xndx);
        if (!sym)
          error("gelf_getsymshndx failed: %s", elf_errmsg(-1));
...
static void set_symbol_addr(struct symbol *sym, void *arg) {
  ...
  if (sym->addr.section == SHN_UNDEF) {
    sym->addr = *addr;
    hash_add(symbol_addrs, &sym->addr_hash, symbol_addr_hash(&sym->addr));
