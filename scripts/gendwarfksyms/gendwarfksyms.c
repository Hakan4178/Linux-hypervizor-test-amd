...
static void usage(void) {
  fputs("Usage: gendwarfksyms [options] elf-object-file ... < symbol-list\n\n"
        ...
        "      --dump-versions  Dump expanded type strings used for symbol "
        "versions\n"
...
int main(int argc, char **argv) {
  ...
  static const struct option opts[] = {{"debug", 0, NULL, 'd'},
  ...
  if (symtypes_file) {
    ...
    if (!symfile)
      error("fopen failed for '%s': %s", symtypes_file, strerror(errno));
  }
  ...
  for (n = optind; n < argc; n++) {
    ...
    if (fd == -1)
      error("open failed for '%s': %s", argv[n], strerror(errno));
    ...
    if (!dwfl)
      error("dwfl_begin failed for '%s': %s", argv[n], dwarf_errmsg(-1));

    if (!dwfl_report_offline(dwfl, argv[n], argv[n], fd))
      error("dwfl_report_offline failed for '%s': %s", argv[n],
            dwarf_errmsg(-1));
