...
static void check_duplicate_node_names(struct check *c, struct dt_info *dti,
  ...
  for_each_child(
      node, child) for (child2 = child->next_sibling; child2;
                        child2 = child2->next_sibling) if (streq(child->name,
                                                                 child2->name))
...
WARNING(node_name_vs_property_name, check_node_name_vs_property_name, NULL,
        &node_name_chars);
  ...
  for_each_property(node, prop) {
    ...
    if (n < strlen(name))
      FAIL_PROP(c, dti, node, prop,
                "Character '%c' not recommended in property name", name[n]);
  }
}
...
#define DESCLABEL_ARGS(node, prop, mark)                                       \
  ((mark) ? "value of " : ""), ((prop) ? "'" : ""),                            \
      ((prop) ? (prop)->name : ""), ((prop) ? "' in " : ""), (node)->fullpath
...
static void check_duplicate_label(struct check *c, struct dt_info *dti,
  ...
  if ((othernode != node) || (otherprop != prop) || (othermark != mark))
    FAIL(c, dti, node,
         "Duplicate label '%s' on " DESCLABEL_FMT " and " DESCLABEL_FMT, label,
         DESCLABEL_ARGS(node, prop, mark),
...
static cell_t check_phandle_prop(struct check *c, struct dt_info *dti,
  ...
  if (prop->val.len != sizeof(cell_t)) {
    FAIL_PROP(c, dti, node, prop, "bad length (%d) %s property", prop->val.len,
              prop->name);
    return 0;
  }
  ...
  if (!phandle_is_valid(phandle)) {
    FAIL_PROP(c, dti, node, prop, "bad value (0x%x) in %s property", phandle,
              prop->name);
    return 0;
  }
...
static void check_explicit_phandles(struct check *c, struct dt_info *dti,
  ...
  if (other && (other != node)) {
    FAIL(c, dti, node, "duplicated phandle 0x%x (seen before at %s)", phandle,
         other->fullpath);
    return;
  }
...
static void check_pci_device_bus_num(struct check *c, struct dt_info *dti,
  ...
  if ((bus_num < min_bus) || (bus_num > max_bus))
    FAIL_PROP(c, dti, node, prop,
              "PCI bus number %d out of range, expected (%d - %d)", bus_num,
              min_bus, max_bus);
}
  ...
  for (len = prop->val.len; len > 0; len -= 4) {
    ...
    if (reg & I2C_TEN_BIT_ADDRESS) {
      if ((reg & ~I2C_TEN_BIT_ADDRESS) > 0x3ff)
        FAIL_PROP(c, dti, node, prop,
                  "I2C address must be less than 10-bits, got \"0x%x\"", reg);
    } else if (reg > 0x7f)
      FAIL_PROP(c, dti, node, prop,
                "I2C address must be less than 7-bits, got \"0x%x\". Set "
                "I2C_TEN_BIT_ADDRESS for 10 bit addresses or fix the property",
                reg);
  }
}
...
static void check_avoid_unnecessary_addr_size(struct check *c,
  ...
  if (get_property(node, "ranges") || get_property(node, "dma-ranges") ||
      !node->children)
    return;
  ...
  FAIL(c, dti, node,
       ...
       "\"dma-ranges\" or child \"reg\" or \"ranges\" property");
}
  ...
  for_each_child(node, childa) {
    ...
    for_each_child(node, childb) {
      ...
      if (streq(addr_a, addr_b))
        FAIL(c, dti, childb, "duplicate unit-address (also used in node %s)",
             childa->fullpath);
    }
  }
}
...
static void check_property_phandle_args(struct check *c, struct dt_info *dti,
  ...
  for (cell = 0; cell < prop->val.len / sizeof(cell_t); cell += cellsize + 1) {
    ...
    if (prop->val.markers) {
      ...
      if (!m)
        FAIL_PROP(c, dti, node, prop, "cell %d is not a phandle reference",
                  cell);
    }

    provider_node = get_node_by_phandle(root, phandle);
    if (!provider_node) {
      FAIL_PROP(c, dti, node, prop, "Could not get phandle node for (cell %d)",
                cell);
      break;
    }
    ...
    if (cellprop) {
      ...
      FAIL(c, dti, node,
           "Missing property '%s' in node %s or bad phandle (referred from "
           "%s[%d])",
    ...
    if ((expected <= cell) || prop->val.len < expected) {
      FAIL_PROP(c, dti, node, prop,
                "property size (%d) too small for cell size %u", prop->val.len,
                cellsize);
      break;
    }
  }
...
#define WARNING_PROPERTY_PHANDLE_CELLS(nm, propname, cells_name, ...)          \
  ...
  WARNING(nm##_property, check_provider_cells_property, &nm##_provider,        \
          &nm##_is_cell, &phandle_references);
...
static bool prop_is_gpio(struct property *prop) {
  ...
  return strends(prop->name, "-gpios") || streq(prop->name, "gpios") ||
         strends(prop->name, "-gpio") || streq(prop->name, "gpio");
}
...
static void check_interrupt_map(struct check *c, struct dt_info *dti,
  ...
  if (node->addr_cells < 0) {
    FAIL(c, dti, node, "Missing '#address-cells' in interrupt-map provider");
    return;
  }
  ...
  if (prop && (prop->val.len != (cellsize * sizeof(cell_t))))
    FAIL_PROP(c, dti, node, prop, "property size (%d) is invalid, expected %zu",
  ...
  for (cell = 0; cell < map_cells;) {
    ...
    if (!phandle_is_valid(phandle)) {
      /* Give up if this is an overlay with external references */
      if (!(dti->dtsflags & DTSF_PLUGIN))
        FAIL_PROP(c, dti, node, irq_map_prop, "Cell %zu is not a phandle(%d)",
                  cell, phandle);
      break;
    }
    ...
    if (!provider_node) {
      FAIL_PROP(c, dti, node, irq_map_prop,
                "Could not get phandle(%d) node for (cell %zu)", phandle, cell);
      break;
    }
    ...
    if (cellprop) {
      ...
      FAIL(c, dti, node,
           "Missing property '#interrupt-cells' in node %s or bad phandle "
           "(referred from interrupt-map[%zu])",
...
static void check_interrupts_property(struct check *c, struct dt_info *dti,
  ...
  if (!is_multiple_of(irq_prop->val.len, irq_cells * sizeof(cell_t))) {
    FAIL_PROP(c, dti, node, prop, "size is (%d), expected multiple of %d",
...
static void check_graph_nodes(struct check *c, struct dt_info *dti,
  ...
  for_each_child(node, child) {
    ...
    if (!node->parent) {
      FAIL(c, dti, node,
           "root node contains endpoint node '%s', potentially misplaced "
