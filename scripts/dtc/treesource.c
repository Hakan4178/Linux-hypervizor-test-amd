 ...
 */
static struct marker **add_marker(struct marker **mi, enum markertype type,
...
void property_add_marker(struct property *prop, enum markertype type,
                         unsigned int offset, char *ref) {
  add_marker(&prop->val.markers, type, offset, ref);
}

static void add_string_markers(struct property *prop, unsigned int offset,
                               int len) {
  int l;
...
void add_phandle_marker(struct dt_info *dti, struct property *prop,
                        unsigned int offset) {
  cell_t phandle;
  struct node *refn;
  char *ref;
  ...
  for_each_marker(m) {
    ...
    case TYPE_UINT32:
      ...
      for_each_marker_of_type(
          m_phandle, REF_PHANDLE) if (m->offset == m_phandle->offset) break;
