...
static int overlay_fixup_phandle(void *fdt, void *fdto, int symbols_off,
  ...
  do {
    ...
    ret = overlay_fixup_one_phandle(fdto, symbols_off, path, path_len, name,
                                    name_len, poffset, phandle);
    if (ret)
      return ret;
  } while (len > 0);
...
static int overlay_prevent_phandle_overwrite_node(void *fdt, int fdtnode,
  ...
  fdt_for_each_subnode(fdtochild, fdto, fdtonode) {
    ...
    ret =
        overlay_prevent_phandle_overwrite_node(fdt, fdtchild, fdto, fdtochild);
    if (ret)
      return ret;
  }
