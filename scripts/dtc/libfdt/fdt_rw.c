...
static int fdt_resize_property_(void *fdt, int nodeoffset, const char *name,
  ...
  *prop = fdt_get_property_namelen_w(fdt, nodeoffset, name, namelen, &oldlen);
  if (!*prop)
    return oldlen;
...
int fdt_setprop_placeholder_namelen(void *fdt, int nodeoffset, const char *name,
  ...
  if (err == -FDT_ERR_NOTFOUND)
    err = fdt_add_property_(fdt, nodeoffset, name, namelen, len, &prop);
  if (err)
    return err;
...
int fdt_setprop_namelen(void *fdt, int nodeoffset, const char *name,
  ...
  err = fdt_setprop_placeholder_namelen(fdt, nodeoffset, name, namelen, len,
                                        &prop_data);
  if (err)
    return err;
...
int fdt_appendprop(void *fdt, int nodeoffset, const char *name, const void *val,
  ...
  if (prop) {
    ...
    err = fdt_add_property_(fdt, nodeoffset, name, strlen(name), len, &prop);
    if (err)
      return err;
