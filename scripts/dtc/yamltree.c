...
static void yaml_propval_int(yaml_emitter_t *emitter, struct marker *markers,
                             char *data, unsigned int seq_offset,
                             unsigned int len, int width) {
  yaml_event_t event;
  void *tag;
  unsigned int off;
  ...
  yaml_sequence_start_event_initialize(&event, NULL, (yaml_char_t *)tag,
                                       width == 4, YAML_FLOW_SEQUENCE_STYLE);
  ...
  for (off = 0; off < len; off += width) {
    ...
    if (is_phandle)
      yaml_scalar_event_initialize(&event, NULL, (yaml_char_t *)"!phandle",
                                   (yaml_char_t *)buf, strlen(buf), 0, 0,
...
static void yaml_propval(yaml_emitter_t *emitter, struct property *prop) {
  ...
  yaml_scalar_event_initialize(&event, NULL, (yaml_char_t *)YAML_STR_TAG,
                               (yaml_char_t *)prop->name, strlen(prop->name), 1,
  ...
  if (len == 0) {
    yaml_scalar_event_initialize(&event, NULL, (yaml_char_t *)YAML_BOOL_TAG,
                                 (yaml_char_t *)"true", strlen("true"), 1, 0,
                                 YAML_PLAIN_SCALAR_STYLE);
    yaml_emitter_emit_or_die(emitter, &event);
    return;
  }
  ...
  yaml_sequence_start_event_initialize(
      &event, NULL, (yaml_char_t *)YAML_SEQ_TAG, 1, YAML_FLOW_SEQUENCE_STYLE);
  yaml_emitter_emit_or_die(emitter, &event);
...
static void yaml_tree(struct node *tree, yaml_emitter_t *emitter) {
  ...
  for_each_child(tree, child) {
    yaml_scalar_event_initialize(
        &event, NULL, (yaml_char_t *)YAML_STR_TAG, (yaml_char_t *)child->name,
