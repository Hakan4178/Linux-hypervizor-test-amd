...
static int append_unique_str_to_property(struct node *node, char *name,
...
static int append_unique_u32_to_property(struct node *node, char *name,
                                         fdt32_t value) {
  struct property *p;
...
struct node *get_subnode(struct node *node, const char *nodename) {
  ...
  for_each_child(node, child) if (streq(child->name, nodename) &&
                                  !child->deleted) return child;
...
static void add_phandle_property(struct node *node, const char *name,
                                 int format) {
  struct data d;
...
static struct node *build_and_name_child_node(struct node *parent,
                                              const char *name) {
  struct node *node;
...
static void generate_label_tree_internal(struct dt_info *dti, struct node *an,
  ...
  if (node->labels) {
    ...
    for_each_label(node->labels, l) {
      ...
      p = build_property(
          l->label,
          data_copy_escape_string(node->fullpath, strlen(node->fullpath)),
          NULL);
      add_property(an, p);
    }
...
static int generate_fixups_tree_internal(struct dt_info *dti, struct node *fn,
                                         struct node *node) {
  struct node *dt = dti->dt;
  struct node *c;
  struct property *prop;
  ...
  for_each_child(node, c) if (generate_fixups_tree_internal(dti, fn, c)) ret =
      -1;
...
static int add_local_fixup_entry(struct dt_info *dti, struct node *lfn,
                                 struct node *node, struct property *prop,
...
static int generate_local_fixups_tree_internal(struct dt_info *dti,
  ...
  for_each_child(node, c) if (generate_local_fixups_tree_internal(dti, lfn, c))
      ret = -1;
...
void generate_label_tree(struct dt_info *dti, const char *name, bool allocph) {
  if (!any_label_tree(dti, dti->dt))
    return;
  generate_label_tree_internal(dti, build_root_node(dti->dt, name), dti->dt,
                               allocph);
}
...
void generate_fixups_tree(struct dt_info *dti, const char *name) {
  if (!any_fixup_tree(dti, dti->dt))
    return;
  if (generate_fixups_tree_internal(dti, build_root_node(dti->dt, name),
                                    dti->dt))
    fprintf(stderr,
            "Warning: Preexisting data in %s malformed, some content could not "
            "be added.\n",
            name);
}
  ...
  for_each_property(an, fp) {
    ...
    while ((fl = fp->val.len - (fnext - fp->val.val))) {
      ...
      *(soffset - 1) = '\0';
      ...
      if (!p && quiet < 1)
        fprintf(stderr,
                "Warning: Label %s references non-existing property %s in node "
                "%s\n",
      ...
      *(soffset - 1) = ':';
      ...
      if (offset < 0 || offset + 4 > p->val.len) {
        if (quiet < 1)
          fprintf(stderr,
                  "Warning: Label %s contains invalid offset for property %s "
                  "in node %s\n",
...
void generate_local_fixups_tree(struct dt_info *dti, const char *name) {
  if (!any_local_fixup_tree(dti, dti->dt))
    return;
  if (generate_local_fixups_tree_internal(dti, build_root_node(dti->dt, name),
                                          dti->dt))
    fprintf(stderr,
            "Warning: Preexisting data in %s malformed, some content could not "
            "be added.\n",
...
static void local_fixup_phandles_node(struct dt_info *dti, struct node *lf,
                                      struct node *n) {
  struct property *lfp;
  ...
  for_each_property(lf, lfp) {
    ...
    if (!p) {
      if (quiet < 1)
        fprintf(stderr,
                "Warning: Property %s in %s referenced in __local_fixups__ "
                "missing\n",
                lfp->name, n->fullpath);
      continue;
    }
    ...
    if (lfp->val.len % sizeof(fdt32_t)) {
      if (quiet < 1)
        fprintf(stderr,
                "Warning: property %s in /__local_fixups__%s malformed\n",
                lfp->name, n->fullpath);
      continue;
    }
  ...
  for_each_child(lf, lfsubnode) {
    ...
    if (!subnode) {
      if (quiet < 1)
        fprintf(stderr,
                "Warning: node %s/%s referenced in __local_fixups__ missing\n",
