...
static void *mmap_file(char const *fname) {
  ...
  file_map =
      mmap(0, sb.st_size, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd_map, 0);
...
static unsigned char push_bl_mcount_thumb_le[6] = {
    0x00, 0xb5, 0xff, 0xf7, 0xfe, 0xff}; /* push {lr}, bl */
static unsigned char push_bl_mcount_thumb_be[6] = {
    0xb5, 0x00, 0xf7, 0xff, 0xff, 0xfe}; /* push {lr}, bl */
...
static uint64_t w8nat(uint64_t const x) { return x; }

static uint32_t w4nat(uint32_t const x) { return x; }

static uint32_t w2nat(uint16_t const x) { return x; }
...
static void MIPS64_r_info(Elf64_Rel *const rp, unsigned sym, unsigned type) {
  rp->r_info =
      ((union mips_r_info){.r_mips = {.r_sym = w(sym), .r_type = type}}).r_info;
}
...
static int do_file(char const *const fname) {
  ...
  if (memcmp(ELFMAG, ehdr->e_ident, SELFMAG) != 0 ||
      w2(ehdr->e_type) != ET_REL || ehdr->e_ident[EI_VERSION] != EV_CURRENT) {
